...
raft_node_id_t raft_get_nodeid (raft_server_t *me_)
{
    ...
    if (!me->node)
    {
        printf("%s %s %d !me->node\n",__FILE__,__FUNCTION__,__LINE__);
        return -1;
    }
    printf("%s %s %d raft_node_get_id\n",__FILE__,__FUNCTION__,__LINE__);
...
raft_node_t* raft_get_my_node (raft_server_t *me_)
{
    raft_server_private_t *me = (raft_server_private_t*) me_;
    int i;
    raft_node_id_t a,b =0;
    printf("%s %s %dme->num_nodes is %d\n",__FILE__,__FUNCTION__,__LINE__,me->num_nodes);

    for (i = 0; i < me->num_nodes; i++)
    {
        a = raft_get_nodeid (me_);
        b= raft_node_get_id (me->nodes[i]);
        printf("%s %s %d a is %d,b is %d\n",__FILE__,__FUNCTION__,__LINE__,a,b);
...
int raft_is_apply_allowed (raft_server_t *me_)
{
    return (!raft_snapshot_is_in_progress (me_)
            || (((raft_server_private_t*) me_)->snapshot_flags
                    & RAFT_SNAPSHOT_NONBLOCKING_APPLY));
}
...
//设置快照
void raft_set_snapshot_metadata (raft_server_t *me_, raft_term_t term,
                                 raft_index_t idx)
{
    raft_server_private_t *me = (raft_server_private_t*) me_;
